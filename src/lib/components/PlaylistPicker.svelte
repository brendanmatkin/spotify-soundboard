...
<div class="modal-overlay" onclick={onclose} role="presentation">
	...
	>
		...
		<div class="tabs">
			<button class:active={pickerTab === 'paste'} onclick={() => (pickerTab = 'paste')}>Paste URL</button>
			<button class:active={pickerTab === 'search'} onclick={() => (pickerTab = 'search')}>Search</button>
			<button class:active={pickerTab === 'library'} onclick={() => { pickerTab = 'library'; loadLibrary(); }}>My Playlists</button>
		</div>
		...
		<div class="modal-body">
			{#if pickerTab === 'paste'}
				<div class="input-row">
					...
					/>
					<button class="btn-accent" onclick={pasteAdd} disabled={loading}>{loading ? '...' : 'Add'}</button>
				</div>
			{:else if pickerTab === 'search'}
				<div class="input-row">
					...
					/>
					<button class="btn-accent" onclick={doSearch} disabled={loading}>{loading ? '...' : 'Search'}</button>
				</div>
				<div class="results">
			...
			{:else}
				<div class="filter-form">
					<input type="text" placeholder="Filter playlists..." bind:value={libraryFilter} class="input-field" />
				</div>
				{#if loading}
...
<style>
	...
	.close-btn:active { background: var(--surface-raised); }
	...
	.input-row :global(.input-field) { flex: 1; }

	.filter-form { margin-bottom: 10px; }
	.filter-form :global(.input-field) { width: 100%; }
	...
	.result-img.placeholder { background: var(--surface-hover); }
